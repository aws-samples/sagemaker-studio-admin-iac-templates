...
def find_efs_volume_managed_by_sagemaker_domain(domain_arn):
    ...
    for volume in efs_volumes:
        for tag in volume["Tags"]:
            if (tag.get("Key") == "ManagedByAmazonSageMakerResource") and (
                tag.get("Value") == domain_arn
            ):
                return volume
    return None
...
def delete_mount_targets_for_file_system(file_system_id):
    ...
    for mount_target in mount_targets:
        ...
        print(
            f"Deleted mount target {mount_target['MountTargetId']} for the EFS volume."
        )
...
def list_security_groups_managed_by_sagemaker_domain(domain_arn):
    ...
    response = ec2_client.describe_security_groups(
        Filters=[
            {"Name": "tag:ManagedByAmazonSageMakerResource", "Values": [domain_arn]}
        ]
    )
...
def delete_security_groups(security_groups):
    ...
    for security_group in security_groups:
        ...
        if sg.ip_permissions:
            ...
            print(
                f"Removed ingress permissions on security group {security_group['GroupId']}"
            )
        if sg.ip_permissions_egress:
            sg.revoke_egress(IpPermissions=sg.ip_permissions_egress)
            print(
                f"Removed egress permissions on security group {security_group['GroupId']}"
            )
    ...
    for security_group in security_groups:
        ...
        run_with_retry(
            lambda: ec2_client.delete_security_group(GroupId=security_group["GroupId"]),
            "DependencyViolation",
        )
...
def run_with_retry(target, expected_error_message):
    ...
    while True:
        try:
            ...
            ) or f"({expected_error_message})" not in str(ex):
                raise ex
            if attempt > 7:
                raise ex
            ...
            print(
                f"Dependent objects still exist. Retrying after {wait_time} seconds..."
            )
...
def list_sagemaker_endpoint_eni(vpc_id):
    ...
    return list(
        filter(
            lambda eni: ":SageMaker" in eni["RequesterId"],
...
def delete_eni(eni_list):
    ...
    for eni in eni_list:
        ...
        run_with_retry(
            lambda: ec2_client.delete_network_interface(NetworkInterfaceId=eni_id),
...
def main():
    ...
    security_groups = list_security_groups_managed_by_sagemaker_domain(
        args.sagemaker_studio_domain
    )
    ...
    efs_volume_managed_by_sagemaker_domain = (
        find_efs_volume_managed_by_sagemaker_domain(args.sagemaker_studio_domain)
    )
    if efs_volume_managed_by_sagemaker_domain is None:
        raise Exception(
            f"Could not find the EFS volume managed by Amazon SageMaker domain {args.sagemaker_studio_domain}"
        )
    print(
        f"Found EFS Volume managed by SageMaker domain {args.sagemaker_studio_domain}. FileSystemId is: {efs_volume_managed_by_sagemaker_domain['FileSystemId']}\n"
    )

    delete_mount_targets_for_file_system(
        efs_volume_managed_by_sagemaker_domain["FileSystemId"]
    )

    delete_security_groups(security_groups)

    print(
        f"If you don't need the data in the EFS volume {efs_volume_managed_by_sagemaker_domain['FileSystemId']} created by SageMaker, delete it from the AWS Console."
    )
